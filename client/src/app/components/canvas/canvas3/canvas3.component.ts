...
@Component({
  ...
  styleUrls: ['./canvas3.component.scss']
})
export class Canvas3Component implements OnInit {
      ...
      if(markers[3] != "a")
      {
        for(let i = 0; i < markers.length; i++)
        {
          if(markers[i][1] == 3)
          {
            ...
            let marker = {id: 3, x: markers[i][3], y: markers[i][4] }
  ...
  newMessage(data: any) 
  {
    this._canvasDataService.changeMessage3(data)
  }
